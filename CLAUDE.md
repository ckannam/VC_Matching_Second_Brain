...
- **Two-rubric dispatch (SHIPPED):** `scoreVC(vc, tech, portfolioCompanies)` in `scoring.js` routes firms WITH a non-empty portfolio array to v2 (`vcFitScore`), and firms WITHOUT to the ported v1 (`vcFitScoreV1`). v1 uses the two-dimension formula `0.60·Sector + 0.40·Stage` (reweighted 2026-08-03 — geography and check-size, both weak/proxy signals, were removed and their weight redistributed; Sector kept dominant so off-domain firms fall below the 0.45 floor) with its own frozen `INDUSTRY_TO_DOMAIN` table (the pre-taxonomy snapshot). The prior degraded-v2 "stated" path for no-portfolio firms is retired. Both `index.html` (`topTechsForVC`, `findVCsForTech`) and `scripts/generate_vc.js` call `scoreVC`. Note: the v1 code is intentionally duplicated — `scoring.js` holds the live/tunable copy (now 2-dim); `scripts/generate_v1_baseline.js` holds a frozen copy of the ORIGINAL 4-dimension v1 (industry 37.5% + stage 30% + check 22.5% + geo 10%) for offline comparison-artifact stability. Do NOT DRY them.
...
```
`domains[]` uses JHTV's 8 domain names (empty = out-of-scope, scores 0); `stage` is a round-ladder string (`Seed`/`Series A`/…, omit if unknown → domain-only credit). **65 entries total, split by `sourceUrl`:** **19 hand-classified** (real firm-website `sourceUrl`s — the 12 curated PDF pilot firms plus a few Juneberry target firms, companies scraped + classified by hand) and **46 derived** (`sourceUrl: "pitchbook-deals"`, built from PitchBook deal history). `PORTFOLIO_BY_VC` drives the v2 path for all of them. The hand-classified entries stay hand-editable; the derived entries are regenerated by `scripts/build_deal_derived.js` (which preserves the hand-classified ones — see the deal-data pipeline below). Not consumed by the backend. Out-of-scope companies add 0 credit and are omitted.
